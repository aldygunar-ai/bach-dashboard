...
def get_gspread_client():
    """Membuat client gspread dengan perbaikan private_key."""
...
def load_stock_per_pltd():
    ...
    for pltd, sheet_id in PLTD_SHEETS.items():
        try:
            ...
            pass  # Abaikan sheet yang belum dibagikan
...
def load_project_data():
    headers = {'User-Agent': 'Mozilla/5.0'}
    df_ops = df_das = pd.DataFrame()
    try:
        resp = requests.get(SHAREPOINT_PROJECT_OPS, headers=headers, timeout=20)
        df_ops = pd.read_excel(io.BytesIO(resp.content))
        df_ops['PROJECT'] = 'PROJECT PLTD'
    except: pass
    try:
        resp = requests.get(SHAREPOINT_PROJECT_DAS, headers=headers, timeout=20)
...
def home():
    ...
    if df_stock.empty:
        st.warning("Data stok belum tersedia. Pastikan semua spreadsheet telah dibagikan ke service account. Coba muat ulang halaman ini.")
        return
...
def stock_pltd():
    ...
    if not df_deliv.empty:
        ...
        kat_list = df_deliv['Kategori'].unique()
        selected_kat = st.multiselect("Kategori", kat_list, default=kat_list)
        df_deliv_f = df_deliv[df_deliv['Kategori'].isin(selected_kat)]
...
def analisis_stok():
    ...
    cols_show = ['PLTD','Kode Material','Nama Material','Jenis','Qty','Kebutuhan/bin (PM)','Kebutuhan/bin (Aktual)','Sisa Hari Stok','Durasi Kirim (hari)','Status','Propose Kirim']
    if 'Harga Satuan' in df_view.columns: cols_show.append('Harga Satuan')
...
def pemakaian():
    ...
    # Mapping kolom adaptif
    col_map = {}
    for col in df.columns:
        l = col.lower()
        if 'tanggal' in l: col_map['Tanggal'] = col
        elif 'pltd' in l or 'site' in l: col_map['PLTD'] = col
        elif 'kode' in l and 'material' in l: col_map['Kode Material'] = col
        elif 'nama' in l and 'material' in l: col_map['Nama Material'] = col
        elif 'qty' in l or 'quantity' in l: col_map['Qty'] = col
        elif 'biaya' in l or 'total' in l or 'cost' in l: col_map['Biaya'] = col
        elif 'transaksi' in l or 'doc' in l or 'no' in l: col_map['No Transaksi'] = col
    if 'Tanggal' not in col_map:
        st.error("Kolom Tanggal tidak ditemukan.")
        return

    df['Tanggal'] = pd.to_datetime(df[col_map['Tanggal']], errors='coerce')
    df['PLTD'] = df[col_map['PLTD']] if 'PLTD' in col_map else 'Unknown'
    ...
    sel_pltd = st.sidebar.multiselect("PLTD", pltd_opt, default=pltd_opt[:5])
    ...
    with col1:
        ...
        fig = px.bar(top_mat, x='Qty', y='Nama Material', orientation='h', text='Qty', color='Qty', color_continuous_scale='Blues')
    ...
    with col2:
        ...
        fig = px.bar(top_site, x='Qty', y='PLTD', orientation='h', text='Qty', color='Qty', color_continuous_scale='Teal')
        fig.update_layout(yaxis={'categoryorder':'total ascending'}, height=400)
        fig.update_traces(textposition='outside')
        st.plotly_chart(fig, use_container_width=True, config={'doubleClick': 'reset'})
    ...
    st.dataframe(need.head(20)[['Tanggal', 'PLTD', 'Kode Material', 'Nama Material', 'Qty', 'Biaya', 'No Transaksi']], use_container_width=True, hide_index=True)

    st.markdown("---")
    st.subheader("📍 Peta Sebaran PLTD")
    df_stock = load_stock_per_pltd()
...
def transaksi_project():
    ...
    if not (sel_proj or sel_year or sel_month or sel_stat or sel_site):
        ...
        fig_tr = px.line(trend_data, x='Tgl_Str', y='Requests', markers=True, text='Requests', color_discrete_sequence=['#0A2540'])
        ...
        with c1:
            ...
            fig_site = px.bar(top_site, x='QTY', y='WH TUJUAN', orientation='h', text='QTY', color='QTY', color_continuous_scale='Teal')
        ...
        with c2:
            ...
            fig_item = px.bar(top_item, x='QTY', y='ITEM NAME', orientation='h', text='QTY', color='QTY', color_continuous_scale='Blues')
        ...
        st.dataframe(df_out[['TANGGAL', 'PROJECT', 'WH TUJUAN', 'ITEM NAME', 'QTY', 'STATUS']].head(15), use_container_width=True, hide_index=True)

        st.subheader("📋 Detail Movement Record & Status")
        st.dataframe(df_f[['TANGGAL', 'PROJECT', 'WH TUJUAN', 'ITEM NAME', 'QTY', 'TOTAL COST', 'STATUS']].head(20), use_container_width=True, hide_index=True)
