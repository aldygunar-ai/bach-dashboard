...
# Urutan material untuk tampilan tabel
URUTAN_MATERIAL = [
...
def load_all():
    ...
    for pltd, sid in PLTD_SHEETS.items():
        try:
            ...
            if is_format_log:
                # Format LOG: kolom C=Keluar, D=Stok, I=Nama Material
                # Ambil baris terakhir per material sebagai stok
                log.append(f"  -> Format LOG terdeteksi")
                i_nama = 8
                i_kode = 10  # PRODUCT IDENTIFICATION
                i_qty = 3    # Stok (Pcs)
                
                # Group by nama material, ambil stok terakhir
                material_stok = {}
                for r in data[1:]:
                    ...
                    # Simpan nilai terakhir per material
                    key = (nama.strip().lower(), kode.strip().upper())
                    if key not in material_stok:
                        material_stok[key] = qty
                    else:
                        material_stok[key] = qty  # overwrite dengan yang terbaru
                
                ok = 0
                for (nama_lower, kode), qty in material_stok.items():
                    nama_asli = nama_lower  # akan dinormalisasi oleh norm()
                    rows.append((pltd.strip().upper(), kode, norm(kode, nama_asli).strip(), qty, get_primary_code(kode)))
                    ok += 1
                log.append(f"  -> {ok} material unik (format LOG)")
                
            else:
                # Format STOK: kolom C=Nama Material, D=Kode Material, I=Qty
                ok = 0
                for r in data[1:]:
                    if len(r) < 9:
                        continue
...
def page_stock():
    ...
    if not prev.empty and m1 is not None:
        ...
        if not sisa_df.empty:
            ...
            sp = sp.reset_index()
            # Tambahkan semua PLTD sebagai kolom
